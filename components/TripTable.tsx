...
import { 
  ...
  Printer, ShieldCheck, Plus, Upload, Calculator, Edit3
} from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden animate-in fade-in duration-700 no-print">
      <div className="p-6 md:p-10 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between bg-white gap-6">
        <div>
          <h4 className="font-black text-xl md:text-3xl text-[#002D62] tracking-tight mb-1">ประวัติการเดินทาง</h4>
          <p className="text-[10px] md:text-sm text-slate-400 font-medium">บันทึกและส่งออกรายงานการปฏิบัติราชการ</p>
        </div>
        <div className="flex flex-wrap gap-2 md:gap-3">
          ...
          >
            <Plus size={16} className="text-amber-400" /> สร้างทริป
          </button>
          <button onClick={() => alert('กำลังส่งออกข้อมูล Excel...')} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 md:px-5 py-2.5 md:py-3 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-xl md:rounded-2xl text-[11px] md:text-sm font-bold hover:bg-emerald-100 transition-all">
        ...
        <table className="w-full text-left min-w-[800px] lg:min-w-full">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="px-6 md:px-10 py-4 md:py-6 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">ทริป / ภารกิจ</th>
              ...
              <th className="px-6 md:px-10 py-4 md:py-6 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">ค่าใช้จ่ายรวม</th>
              ...
              <tr key={trip.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {(trip.participants || []).map((p, i) => (
                        <span key={i} className="text-[9px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md font-bold border border-indigo-100">{maskName(p)}</span>
                      ))}
                    </div>
                  </div>
                </td>
                <td className="px-6 md:px-10 py-5 md:py-8">
                  <div className="flex items-center space-x-2 md:space-x-3 text-[11px] md:text-sm">
                    <div className="flex flex-col max-w-[120px]">
                       <span className="text-slate-400 font-bold uppercase text-[8px] tracking-widest">Start</span>
                    ...
                    <div className="flex flex-col max-w-[120px]">
                       <span className="text-slate-400 font-bold uppercase text-[8px] tracking-widest">End</span>
                ...
                <td className="px-6 md:px-10 py-5 md:py-8 text-center">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[8px] md:text-[10px] text-slate-400 font-bold uppercase">
                      {trip.distanceKm} KM
                    </span>
                  </div>
                </td>
                <td className="px-6 md:px-10 py-5 md:py-8">
                  <div className="flex items-center gap-1.5 md:gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-all">
                    <button onClick={() => handleEditClick(trip)} className="p-2 md:p-3 bg-white border border-slate-200 rounded-lg md:rounded-xl hover:text-amber-600 shadow-sm" title="แก้ไข"><Edit3 size={16} /></button>
                    <button onClick={() => alert('PDF Report Generated')} className="p-2 md:p-3 bg-white border border-slate-200 rounded-lg md:rounded-xl hover:text-indigo-600 shadow-sm" title="พิมพ์"><Printer size={16} /></button>
                    <button onClick={() => confirm('ลบทริปนี้?') && onDelete(trip.id)} className="p-2 md:p-3 bg-white border border-slate-200 rounded-lg md:rounded-xl hover:text-red-500 shadow-sm" title="ลบ"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
